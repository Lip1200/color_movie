...
# Lecture des données
df_basics = read_tsv('title.basics.filtered.tsv.gz')
df_crew = read_tsv('title.crew.filtered.tsv.gz')
df_episode = read_tsv('title.episode.filtered.tsv.gz')
df_people = read_tsv('name.basics.tsv.gz')
df_principals = read_tsv('title.principals.filtered.tsv.gz')
df_ratings = read_tsv('title.ratings.filtered.tsv.gz')
...
df_metrage.to_csv('metrage.filtered.csv.gz', index=False, compression='gzip')
...
df_film.to_csv('film.filtered.csv.gz', index=False, compression='gzip')
...
df_serie.to_csv('serie.filtered.csv.gz', index=False, compression='gzip')
...
df_episode_export.to_csv('episode.filtered.csv.gz', index=False, compression='gzip')
...
df_people_export.to_csv('personne.filtered.csv.gz', index=False, compression='gzip')

# Trier et limiter les entrées dans la distribution à 10 par titre en utilisant le champ 'ordering'
df_principals_sorted = df_principals.sort_values(by=['tconst', 'ordering'])
df_limited_distribution = df_principals_sorted.groupby('tconst').head(10)
# Préparation des tables Distribution et Distribution_Personne
df_distribution = df_limited_distribution[['tconst', 'nconst', 'category']].drop_duplicates()
df_distribution['ID_Distribution'] = pd.factorize(df_distribution['tconst'])[0] + 1  # Création d'un ID unique simplifié
df_distribution.to_csv('distribution.filtered.csv.gz', index=False, compression='gzip')
...
df_distribution_personne.to_csv('distribution_personne.filtered.csv.gz', index=False, compression='gzip')
